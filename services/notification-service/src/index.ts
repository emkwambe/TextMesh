...
async function main() {
  ...
  // Initialize and start digest job
  const digestJob = new DigestJob(prisma, logger);
  digestJob.start();
  ...
  eventBus.on(EventType.USER_FOLLOWED, async (event) => {
    ...
    await createNotification(prisma, preferenceService, payload.followeeId, 'NEW_FOLLOWER', {
      type: 'new_follower',
  ...
  eventBus.on(EventType.POST_LIKED, async (event) => {
    const payload = event.payload as { postId: string; userId: string; postAuthorId: string; groupId?: string };
    ...
    const [liker, post] = await Promise.all([
      ...
      prisma.post.findUnique({ where: { id: payload.postId }, select: { content: true, groupId: true } }),
    ]);
    if (!liker || !post) return;

    await createNotification(prisma, preferenceService, payload.postAuthorId, 'POST_LIKE', {
      type: 'post_like',
  ...
  eventBus.on(EventType.POST_REPLIED, async (event) => {
    ...
    await createNotification(prisma, preferenceService, payload.postAuthorId, 'POST_REPLY', {
      type: 'post_reply',
      postId: payload.postId,
      postContent: post.content.substring(0, 100),
  ...
  eventBus.on(EventType.GROUP_INVITE_SENT, async (event) => {
    ...
    await createNotification(prisma, preferenceService, payload.inviteeId, 'GROUP_INVITE', {
      type: 'group_invite',
  ...
  // Mount preference routes
  app.use('/preferences', preferenceRoutes(prisma, logger));
...
async function createNotification(
  ...
  // Always create the notification in database
  await prisma.notification.create({ data: { userId, type, payload } });

  // Check if we should send immediate push notification
  const shouldSendImmediate = await preferenceService.shouldSendImmediateNotification(
    userId,
    groupId
  );
  ...
  if (shouldSendImmediate) {
    ...
    logger.debug('Would send immediate push notification', { userId, type, groupId });
  } else {
    // Notification will be sent in digest
    logger.debug('Notification will be sent in digest', { userId, type, groupId });
  }
}
